...
config→running→interview→result; karta pytania pokazuje pod treścią `kontekst` = dosłowny cytat z ogłoszenia], `paywall-dialog.tsx` [TRZY widoki: `cennik` (dwa plany z `PLANY` × dwa okresy) → `potwierdzenie` (podsumowanie wybranego planu + OBIE zgody + zapłata) oraz `jednorazowo` = down-sell „to jedno dopasowanie za 12 zł" przy wyjściu z obu poprzednich (wymaga propa `tailoringId`, bez niego down-sell się nie pokazuje, bo nie ma czego odblokować). **Krok `potwierdzenie` dodany 2026-08-13 z powodu przestrzennego, nie prawnego:** zgody trzymane na stałe pod cennikiem zjadały na telefonie tyle wysokości, że na karty planów zostawała szczelina ~150 px. Rozbicie odwzorowuje przy okazji kolejność z § 4 ust. 8 Regulaminu (wybierz wariant → złóż oświadczenia → zapłać); wcześniej dało się zaznaczyć zgody PRZED wyborem planu. `DialogContent` = flex-col + przewijany TYLKO środek; w `cennik` i `jednorazowo` nagłówek jedzie RAZEM z treścią (krzyżyk jest `absolute` względem `DialogContent`, więc zostaje), w `potwierdzenie` stoi. Gałąź `jednorazowo` była wcześniej w całości `shrink-0` wewnątrz `overflow-hidden` — na niskim ekranie treść nie przewijała się, tylko znikała pod krawędzią razem z przyciskiem zakupu], `score-breakdown.tsx`
...
**Zakup** (`paywall-dialog.tsx`, komponent `ZgodyZakupu`) wymaga OBU i gatuje
przyciski PŁATNOŚCI — od 2026-08-13 są to „Płacę …" w kroku `potwierdzenie`
oraz przycisk down-sellu 12 zł; przycisk „Wybieram <plan>" w cenniku tylko
przechodzi do kroku 2 i zgód nie wymaga, bo nie zawiera jeszcze umowy.
Zgody resetują się przy każdym zamknięciu okna ORAZ przy zejściu do down-sellu
— zgoda nie ma prawa „zostać zaznaczona" z poprzedniej wizyty ani przenieść się
na inny produkt niż ten, przy którym padła.
...
- **Konfiguracja Stripe'a POZA kodem (zmienne, ceny, webhook, Billing Portal)** → **`STRIPE.md`**: siedem zmiennych i ich scope per środowisko, PIĘĆ zdarzeń webhooka, zweryfikowany stan sandboxa z identyfikatorami cen, plan dla live i checklista audytu. Ten plik istnieje, bo konfiguracja płatności żyje w panelach (Stripe + Vercel), więc nic w repo jej nie wymusza i nic nie krzyczy, gdy jest niepełna — dwie awarie już z tego wyszły (brak `STRIPE_WEBHOOK_SECRET` na Vercelu 2026-08-04, zmienne tylko w scope Preview 2026-08-13)

...
<p class="image-prompt">📷 <strong>GRAFIKA 2 (infographic)</strong> - prompt: A clean minimal infographic showing a single page CV layout as a wireframe with gray placeholder text blocks for header, experience, and skills sections, with the bottom footer area highlighted in a soft accent color and a small text line labeled clearly as the RODO consent clause, flat design, plenty of white space, professional blue and white color scheme, no real text, vector style. <strong>Alt:</strong> "Diagram strony CV z zaznaczonym miejscem na klauzulę RODO w stopce". <strong>Podpis:</strong> "Klauzula RODO trafia do stopki, mniejszą czcionką niż reszta CV". Plik: <code>klauzula-rodo-w-cv-02.webp</code>. <strong>Po wgraniu obrazka usuń ten akapit.</strong></p>
...
<p>Sama nieaktualna klauzula rzadko przekreśla aplikację - to drobiazg. Ale w zestawieniu z innymi (stare formaty daty, nieaktualne nazwy stanowisk) buduje wrażenie CV odgrzewanego sprzed lat. W kreatorze <a href="/">Aplikando</a> klauzula jest wstawiana automatycznie w aktualnej wersji, więc nie trzeba tego pilnować przy każdej aktualizacji dokumentu.</p>
